...
static const uint8 Xtime2Sbox[256] = {
...
static const uint8 Xtime3Sbox[256] = {
...
static void MixSubColumns(uint8 * state);
static void InvMixSubColumns(uint8 * state);
static void AddRoundKey(uint8 * state, uint8 * key);

/* function body */
/*
*	Function:  ShiftRows
*	Action:    S-box变换和行变换
*	Input:     state  - 4*4的状态表首指针
*	Output:    row0 - 位置不变        row1 - 向左循环移动1位
...
*	Action:    反S-box变换和反行变换
*	Input:     state  - 4*4的状态表首指针
*	Output:    row0 - 位置不变         row1 - 向右循环移动1位
...
* NOTE:     列混合操作。事实上是一种替代操作，用 State 字节列的值进行数学域加
*           和域乘的结果代替每个字节。
*           包括S-box变换，行变换，列变换
*           MixColumns 使用特殊的加法和乘法，是基于近代数学的域论的有限域GF(28).
...
*******************************************************************************/
/*
*	Function:  MixSubColumns
*	Action:    列混合操作,包括S-box变换，行变换，列变换
*	Input:     state  - 4*4的状态表首指针
*	Output:    none
*/
static void MixSubColumns(uint8 *state)
{
	uint8 newstate[4 * Nc];    //用于缓存新的"state"数组
	uint8 i;
...
/*
*	Function:  InvMixSubColumns
*	Action:    反向列混合操作,包括S-box变换，行变换，列变换
*	Input:     state  - 4*4的状态表首指针
*	Output:    none
*/
static void InvMixSubColumns(uint8 *state)
...
static void AddRoundKey(uint8 *state, uint8 *key)
{
    ...
    for (idx=0; idx<4; idx++)
    {
		state[4*idx+0] ^= key[4*idx+0];
		state[4*idx+1] ^= key[4*idx+1];
		state[4*idx+2] ^= key[4*idx+2];
		state[4*idx+3] ^= key[4*idx+3];
    }
}

/* AES-128-bit Public functions ----------------------------------------------------------*/
/*
*	Function:  AES_ExpandKey
*	Action:    扩展密钥操作，每轮扩展Nk字节
*	Input:     key      - 密钥表首指针
*			   expKey   - 扩展密钥首指针
*	Output:    none
*/
void AES_Expandkey( uint8 *key, uint8 *expKey)
{
...
*	Action:    每次加密128Bit数据块
*	Input:     in       -  明文输入缓冲区指针
*              expKey   -  扩展密钥表首指针
*              out      -  密文输出缓冲区指针
*	Output:    密文
*/
void AES_Encrypt( uint8 *in, uint8 *expKey, uint8 *out)
{
	uint8  state[Nc * 4];  //state数组，用于数据分组
	uint16 round;          //加密轮数计数器
	uint16 idx;            //索引
    ...
    /* 与扩展密钥进行“异或”操作 */
    AddRoundKey ((uint8 *)state, (uint8 *)expKey);
    ...
    for (round=1; round<(Nr + 1); round++)
    {
        ...
        /* 与扩展密钥进行“异或”操作 */
        AddRoundKey ((uint8 *)state, (uint8 *)(expKey + round * Nc *4));
    }
...
*	Action:    每次解密128Bit数据块
*	Input:     in       -  密文输入缓冲区指针
*              expKey   -  扩展密钥表首指针
*              out      -  明文输出缓冲区指针
*	Output:    明文
*/
void AES_Decrypt( uint8 *in, uint8 *expKey, uint8 *out)
{
	uint8  state[Nc * 4];  //state数组，用于加密数据分组
	uint16 round;          //加密轮数计数器
	uint16 idx;            //索引
    ...
    /* 与扩展密钥进行“异或”操作 */
    AddRoundKey (state, expKey + (uint16)Nr * Nc *4);
    round = Nr;
