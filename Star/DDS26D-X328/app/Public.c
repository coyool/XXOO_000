...
*/
/*
const uint16_t crc16_d_dlms[16]=     
{    
...
/*uint16 PublicCrc16(const uint8 *pBuf, uint16 Len)
{
	...
	while (Len--)
	{
		fcs = (fcs >> 8) ^ fcstab[(fcs ^ *pBuf++) & 0xff];
	}
	fcs = fcs ^ 0xffff;
	return fcs;
}
*/
uint16 PublicCrc16(const uint8 *ptr, uint16 len)
{
	uint8 db;
	uint16 crc= 0xffff;
...
void PublicDelayUs(uint16 us)
{
	...
	n=(vuint32)us*((SystemCoreClock/5000000UL)+1);	//5000000UL针对每个单片机需要调试
	for(;n>0;n--)
	{
		//DRV_WD_FeedDog();
	}
