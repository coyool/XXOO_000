...
void  OSInit (void)
{
    OSInitHookBegin();  /* 调初始化钩子函数，可加入用户代码 */   /* Call port specific initialization code   */

    OS_InitMisc();      /* 初始化部分全局变量(基础参数初始化) */ /* Initialize miscellaneous variables       */

    OS_InitRdyList();   /* 初始化任务就绪表  */                  /* Initialize the Ready List                */

    OS_InitTCBList();   /* 任务控制块初始化 */                   /* Initialize the free list of OS_TCBs      */

    OS_InitEventList(); /* 初始化ECB链表 */                                         /* Initialize the free list of OS_EVENTs    */
    ...
    OS_MemInit();       /* 初始化内存分区、内存管理 */           /* Initialize the memory manager            */
#endif
...
{
#if OS_TIME_GET_SET_EN > 0
    OSTime        = 0L;                                    /* Clear the 32-bit system clock            */  /* 系统当前时间（节拍数）,清除32 */
#endif

    OSIntNesting  = 0;                                     /* Clear the interrupt nesting counter      */  /* 清除中断嵌套(层数)计数器,中断嵌套最多允许255层  */
    OSLockNesting = 0;                                     /* Clear the scheduling lock counter        */  /* 清除上锁嵌套计数器  */

    OSTaskCtr     = 0;                                     /* Clear the number of tasks                */  /* 清除任务计数器 */

    OSRunning     = OS_FALSE;                              /* Indicate that multitasking not started   */  /* 表明这个多任务系统没有启动，运行的标志，处于不运行状态 */ 

    OSCtxSwCtr    = 0;                                     /* Clear the context switch counter         */  /* 上下文切换的次数(统计任务计数器)清0 */ 
    OSIdleCtr     = 0L;                                    /* Clear the 32-bit idle counter            */  /* 清除32位空闲任务计数器 */

#if OS_TASK_STAT_EN > 0
    OSIdleCtrRun  = 0L;									   /* 空闲任务每秒的计数值清0 */
    OSIdleCtrMax  = 0L;									   /* 空闲任务每秒计数的最大值清0 */
    OSStatRdy     = OS_FALSE;                              /* Statistic task is not ready              */  /* 统计任务是否就绪的标志为空  */
#endif
}
/*$PAGE*/
/*
