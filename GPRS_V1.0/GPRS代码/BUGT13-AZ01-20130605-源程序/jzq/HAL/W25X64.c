...
void W25X64_Test(void)
{
	...
	if(tested == 0)
	{
		...
		int i;
		int xxx = 0;
		unsigned addr = 10*4096;
		PrintLog(LOGTYPE_DEBUG,"w25x64 read test\r\n");
//		w25x64_init();
		if(xxx == 1)
			w25x64_sector_erase(10);
		w25x64_program(addr,&mask,4);
		while(addr<0x04000000)
		{
			PrintLog(LOGTYPE_DEBUG,"addr:%d ",addr);
			w25x64_read(addr,buf,256);
			for(i=0;i<sizeof(buf)/sizeof(buf[0]);i++)
				PrintLog(LOGTYPE_DEBUG," %02X",buf[i]);
